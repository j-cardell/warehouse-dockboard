...
async function checkAuthStatus() {
  ...
  try {
    ...
    authState.multiFacilityMode = data.multiFacilityMode;

    // Check if facility selection is required
    if (data.selectFacilityRequired && data.availableFacilities?.length > 0) {
      showFacilitySelectorModal(data.availableFacilities);
      return true;
    }
...
async function login(username, password) {
  try {
    ...
    if (data.passwordResetRequired) {
      // Store the temporary token and show password reset modal
      authState.tempToken = data.token;
      authState.tempUser = data.user;
    ...
    if (data.token) {
      ...
      showToast(data.message || `Welcome ${data.user?.username || data.user}!`, 'success');
      return true;
    }
  } catch (e) { 
...
// Show facility selector modal for bootstrap users
function showFacilitySelectorModal(facilities, isSwitching = false) {
  const modal = document.getElementById('modal-select-facility');
  const list = document.getElementById('facility-select-list');
  const header = modal?.querySelector('.modal-header h2');
  if (!modal || !list) return;

  // Update header based on mode
  if (header) {
    header.textContent = isSwitching ? 'Switch Facility' : 'Select Facility';
  }

  // Clear existing content
  list.innerHTML = '';

  const currentFacilityId = authState.user?.currentFacility;
  const homeFacilityId = authState.user?.homeFacility;

  // Sort facilities: home first, then current, then alphabetically
  ...
  sortedFacilities.forEach(facility => {
    ...
    // Container for the facility row
    const container = document.createElement('div');
    container.style.cssText = 'display: flex; align-items: center; margin-bottom: 0.5rem; gap: 0.5rem;';

    // Star button for setting home facility
    const starBtn = document.createElement('button');
    starBtn.className = 'btn btn-secondary';
    starBtn.style.cssText = 'padding: 1rem; min-width: 50px; flex-shrink: 0;';
    starBtn.title = isHome ? 'This is your home facility' : 'Click to set as home facility';
    starBtn.innerHTML = isHome ? '⭐' : '☆';
    starBtn.disabled = isHome;
    if (!isHome) {
      starBtn.onclick = async (e) => {
        e.stopPropagation();
        await setHomeFacility(facility.id);
      };
    }

    // Main facility button
    const btn = document.createElement('button');
    btn.className = isCurrent ? 'btn btn-primary' : 'btn btn-secondary';
    btn.style.cssText = 'flex: 1; text-align: left; padding: 1rem;';
    // Disable current facility only in switching mode, not in selection mode
    btn.disabled = isCurrent && isSwitching;

    const currentLabel = isCurrent ? ' <span style="margin-left: 0.5rem;">✓ Current</span>' : '';
    const homeLabel = isHome && !isCurrent ? ' <span style="margin-left: 0.5rem; color: var(--accent-primary);">(Home)</span>' : '';
    const selectLabel = isCurrent && !isSwitching ? ' <span style="margin-left: 0.5rem;">(Click to enter)</span>' : '';
    btn.innerHTML = `<strong>${escapeHtml(facility.id)}</strong>${currentLabel}${homeLabel}${selectLabel}`;

    // Allow clicking if not current in switching mode, OR if current in selection mode
    if (!isCurrent || !isSwitching) {
      btn.onclick = () => {
        if (isSwitching) {
          switchFacility(facility.id);
          closeModal('modal-select-facility');
        } else {
  ...
  // Add "Create New Facility" button for bootstrap admins
  if (authState.user?.isBootstrap) {
    const createFacilityContainer = document.createElement('div');
    createFacilityContainer.style.cssText = 'margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--border);';

    const createFacilityBtn = document.createElement('button');
    createFacilityBtn.className = 'btn btn-success';
    createFacilityBtn.style.cssText = 'width: 100%; padding: 0.75rem;';
    createFacilityBtn.innerHTML = '➕ Create New Facility';
    createFacilityBtn.onclick = () => {
      closeModal('modal-select-facility');
      showSetupModal(true); // true = isNewFacility mode
    };

    createFacilityContainer.appendChild(createFacilityBtn);
    list.appendChild(createFacilityContainer);
  }

  // Add cancel button for switching mode
  if (isSwitching) {
    const cancelBtn = document.createElement('button');
    cancelBtn.className = 'btn';
    cancelBtn.style.cssText = 'width: 100%; margin-top: 1rem; padding: 0.75rem;';
    cancelBtn.textContent = 'Cancel';
    cancelBtn.onclick = () => closeModal('modal-select-facility');
...
async function selectFacility(facilityId) {
  try {
    ...
    if (!res.ok) {
      const data = await res.json();
      throw new Error(data.error || 'Failed to select facility');
    }
    ...
    updateAuthUI();
    showToast(`Welcome to ${data.user.currentFacility}!`, 'success');
    await fetchState();
    await loadSettings();
    renderAll();
  } catch (err) {
...
// Set home facility for the current user
async function setHomeFacility(facilityId) {
  try {
    const res = await fetch('/api/auth/set-home-facility', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${authState.token}`
    ...
    if (!res.ok) {
      const data = await res.json();
      throw new Error(data.error || 'Failed to set home facility');
    }
    ...
    // Refresh the facility selector if it's open
    const modal = document.getElementById('modal-select-facility');
    if (modal?.classList.contains('active')) {
      // Re-render the facility list with updated home facility
      showFacilitySelectorModal(authState.availableFacilities || [], true);
    }

    showToast(data.message || `Home facility set to ${facilityId}`, 'success');
  } catch (err) {
    showToast(err.message, 'error');
  }
}

// Show password reset modal (admin-initiated)
function showPasswordResetModal() {
  // Hide login screen
  const loginScreen = document.getElementById('login-screen');
  if (loginScreen) loginScreen.classList.add('hidden');

  // Create or show password reset modal
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal-content" style="max-width: 400px;">
        <div class="modal-header" style="flex-direction: column; align-items: flex-start; gap: 0.5rem;">
          <h2 style="margin: 0;">🔐 Set New Password</h2>
        </div>
        <div class="modal-body">
          <p class="modal-subtitle" style="margin-bottom: 1.5rem;">Your password has been reset by an administrator. Please set a new password to continue.</p>
          <form id="password-reset-form">
            <div class="form-group">
              <label for="new-password-input">New Password</label>
              <input type="password" id="new-password-input" required minlength="8" placeholder="Enter new password (min 8 characters)">
              <span class="form-hint">Must be at least 8 characters</span>
            </div>
            <div class="form-group">
              <label for="confirm-password-input">Confirm Password</label>
              <input type="password" id="confirm-password-input" required minlength="8" placeholder="Confirm new password">
            </div>
            <div id="password-reset-error" class="login-error hidden"></div>
    ...
    // Add submit handler
    document.getElementById('password-reset-form').addEventListener('submit', handlePasswordResetSubmit);
  } else {
    modal.classList.add('active');
...
async function handlePasswordResetSubmit(e) {
  ...
  try {
    const res = await fetch('/api/auth/set-new-password', {
      ...
      body: JSON.stringify({ token: authState.tempToken, newPassword })
    });
      ...
      <div class="modal-body">
        <form id="change-password-form">
          ...
          <div class="form-group">
            ...
            <input type="password" id="confirm-new-password" required minlength="8" placeholder="Confirm new password">
          </div>
          <div id="change-password-error" class="login-error hidden" style="margin-top: 1rem;"></div>
          <div class="form-actions" style="margin-top: 1.5rem;">
            <button type="button" class="btn btn-secondary" id="btn-cancel-change-password">Cancel</button>
...
function updateAuthUI() {
  ...
  if (authState.isAuthenticated) {
    ...
    // authState.user is now an object with username, role, etc.
    const username = authState.user?.username || authState.user || 'User';
    ...
    const demoBtn = document.getElementById('btn-generate-demo');
    if (demoBtn) {
      if (authState.user?.isBootstrap) {
        demoBtn.classList.add('bootstrap-visible');
      } else {
        demoBtn.classList.remove('bootstrap-visible');
      }
    }

    // Show/hide facility management button (bootstrap only)
    const manageFacilitiesBtn = document.getElementById('btn-manage-facilities');
    if (manageFacilitiesBtn) {
      if (authState.user?.isBootstrap) {
        manageFacilitiesBtn.classList.remove('hidden');
      } else {
        manageFacilitiesBtn.classList.add('hidden');
      }
    }
    ...
    console.log('[AuthUI] Header updated for authenticated user:', username);
  } else {
  ...
  if (authState.isAuthenticated) {
    ...
    checkSetupStatus().then(setupStatus => {
      if (setupStatus.setupNeeded) {
        console.log('[AuthUI] Setup needed, user role:', authState.user?.role);
        // Only admins can create facility
        if (authState.user?.role === 'admin') {
          if (loginScreen) loginScreen.classList.add('hidden');
          showSetupModal();
        } else {
          // Non-admin must wait for admin to create facility
          if (loginScreen) loginScreen.classList.add('hidden');
          showWaitingForAdminModal();
        }
      } else {
...
function updateFacilityUI() {
  ...
  const dropdownFacility = document.getElementById('dropdown-facility');
  if (dropdownFacility) {
    const currentFacility = authState.user?.currentFacility;
    const homeFacility = authState.user?.homeFacility;
    const isMultiFacility = authState.multiFacilityMode;
    const isVisiting = authState.user?.isVisiting;

    if (currentFacility && isMultiFacility) {
      dropdownFacility.classList.remove('hidden');
      const facilityId = getFacilityName(currentFacility);
      if (isVisiting) {
        dropdownFacility.innerHTML = `Facility: ${escapeHtml(facilityId)} <span style="color: var(--accent-warning);">(visiting)</span>`;
      } else {
        dropdownFacility.textContent = `Facility: ${facilityId}`;
      }
    } else {
      dropdownFacility.classList.add('hidden');
    }
  }

  // Show/hide facility switcher button for admins in multi-facility mode
  const switchFacilityBtn = document.getElementById('btn-switch-facility');
  if (switchFacilityBtn) {
    const isAdmin = authState.user?.role === 'admin' || authState.user?.role === 'bootstrap';
    const hasMultipleFacilities = authState.availableFacilities?.length > 1;
    console.log('[updateFacilityUI] Button check:', { isAdmin, multiFacility: authState.multiFacilityMode, hasMultipleFacilities, userRole: authState.user?.role });
    if (isAdmin && authState.multiFacilityMode && hasMultipleFacilities) {
      switchFacilityBtn.classList.remove('hidden');
      console.log('[updateFacilityUI] Button shown');
    } else {
      switchFacilityBtn.classList.add('hidden');
      console.log('[updateFacilityUI] Button hidden');
    }
  }
}

function getFacilityName(facilityId) {
  if (!authState.availableFacilities) return facilityId;
  const facility = authState.availableFacilities.find(f => f.id === facilityId);
...
async function switchFacility(facilityId) {
  try {
    ...
    console.log('[SwitchFacility] Response:', { status: res.status, data });

    // Handle user not in facility - show confirmation to copy
    // Check both status code and response data.code
    const isNotFound = res.status === 404 || String(res.status).startsWith('4');
    const isUserNotInFacility = data.code === 'USER_NOT_IN_FACILITY';

    if (isNotFound || isUserNotInFacility) {
      console.log('[SwitchFacility] Detected user not in facility, canCreate:', data.canCreate, 'status:', res.status);
      if (data.canCreate === true || data.canCreate === 'true') {
        // Get facility name
        // Show confirmation dialog - use facility ID only
        const homeFacilityId = authState.user?.homeFacility || 'unknown';
        let confirmed;
        try {
          confirmed = await showConfirmDialog(
            'Add Yourself to Facility?',
            `You don't exist as a user in "${facilityId}". Would you like to add yourself as an admin to this facility?\n\nUse your existing password to log in. Your actions will be tracked with "@${homeFacilityId}" suffix.`
          );
          console.log('[SwitchFacility] Dialog result:', confirmed);
        } catch (dialogErr) {
          console.error('[SwitchFacility] Dialog error:', dialogErr);
          throw dialogErr;
        }

        if (confirmed) {
          return await switchFacilityWithCreate(facilityId);
        }
        return;
      }
      throw new Error(data.error || 'Failed to switch facility');
    }
    ...
    showToast(`Switched to ${data.user.currentFacility}`, 'success');
    updateAuthUI(); // Re-render UI including button visibility
...
// Switch facility with createIfNotExists flag
async function switchFacilityWithCreate(facilityId) {
  try {
    const res = await fetch('/api/auth/switch-facility', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ facilityId, createIfNotExists: true })
    });

    if (!res.ok) {
      const data = await res.json();
      throw new Error(data.error || 'Failed to create user in facility');
    }
    ...
    showToast(`Added to ${data.user.currentFacility} and switched`, 'success');
    updateAuthUI(); // Re-render UI including button visibility
...
function showConfirmDialog(title, message) {
  ...
  return new Promise((resolve) => {
    // Check if we have a confirmation modal
    let modal = document.getElementById('modal-confirm');
    console.log('[showConfirmDialog] Existing modal found:', !!modal);

    // Create modal if it doesn't exist
    if (!modal) {
      console.log('[showConfirmDialog] Creating new modal');
      modal = document.createElement('div');
      ...
      `;
      document.body.appendChild(modal);
      console.log('[showConfirmDialog] Modal appended to body');
    }

    document.getElementById('confirm-title').textContent = title;
    document.getElementById('confirm-message').textContent = message;

    console.log('[showConfirmDialog] Opening modal');
    ...
    console.log('[showConfirmDialog] Attaching event listeners');
    document.getElementById('confirm-cancel')?.addEventListener('click', onCancel);
    document.getElementById('confirm-ok')?.addEventListener('click', onOk);
    console.log('[showConfirmDialog] Event listeners attached');
...
async function loadSettings() {
  try {
    // Always load global settings first as the base
    const globalData = await apiCall('/settings');
    if (globalData) {
      settings = { ...settings, ...globalData };
    }

    // Then merge user-specific settings if authenticated (overrides global)
    if (authState.isAuthenticated && authState.token) {
      const data = await apiCall('/user/settings');
      if (data?.settings) {
        // Deep merge: user settings override global, but preserve global nested values
        settings = deepMergeSettings(settings, data.settings);
      }
    }
...
// Deep merge helper for settings (preserves global defaults for missing nested properties)
  ...
  for (const key in source) {
    if (source.hasOwnProperty(key)) {
      if (source[key] && typeof source[key] === 'object' && !Array.isArray(source[key]) &&
        ...
        // Deep merge nested objects (user values override target values)
...
function applyGridSettings() {
    ...
    if (grid) {
        ...
        if (s) {
            ...
            if (s.trailerNumber?.color) document.body.style.setProperty('--number-color', s.trailerNumber.color);
            if (s.carrier?.color) document.body.style.setProperty('--carrier-color', s.carrier.color);
            if (s.loadNumber?.color) document.body.style.setProperty('--load-color', s.loadNumber.color);
        }
    }
}
...
async function saveSettings(newSettings) {
  try {
    // Save to user-specific settings if authenticated
    if (authState.isAuthenticated && authState.token) {
      await apiCall('/user/settings', 'POST', newSettings);
    } else {
      // Unauthenticated users: save to global settings
...
function updateSettingsPreview() {
  ...
  // Live tag uses ::after pseudo-element with CSS variables for sizing
  // Set CSS variables on preview container to match settings
...
function openSettingsModal() {
  ...
  document.getElementById('btn-reset-settings')?.addEventListener('click', async () => {
    if (!await showConfirmModal({ title: 'Reset Settings', html: '<p style="color: var(--text-secondary); margin: 0;">Reset all display settings to default?</p>', type: 'warning', confirmText: 'Reset', cancelText: 'Cancel' })) return;
    const defaults = {
      gridColumns: 10,
      trailerDisplay: {
...
function positionsMatch(pos1, pos2) {
  ...
  // Fallback: check unassigned flag when no specific position is set
...
function performSearch(query) {
  ...
  // Search in staging
  if (state.staging) {
    const t = state.staging;
    if (t.number?.toLowerCase().includes(searchQuery) ||
        t.carrier?.toLowerCase().includes(searchQuery) ||
        t.customer?.toLowerCase().includes(searchQuery) ||
        t.loadNumber?.toLowerCase().includes(searchQuery) ||
        'staging'.includes(searchQuery)) {
      searchResults.push({ type: 'staging', trailer: t });
    }
  }
  ...
  // Search in FCFS queue
  (state.queuedTrailers || []).forEach(t => {
    if (t.number?.toLowerCase().includes(searchQuery) ||
        t.carrier?.toLowerCase().includes(searchQuery) ||
        t.customer?.toLowerCase().includes(searchQuery) ||
        t.loadNumber?.toLowerCase().includes(searchQuery) ||
        (t.targetDoorNumber && `door ${t.targetDoorNumber}`.includes(searchQuery))) {
      searchResults.push({ type: 'queue', trailer: t });
    }
  });

  // Search in appointment queue
  (state.appointmentQueue || []).forEach(t => {
    if (t.number?.toLowerCase().includes(searchQuery) ||
        t.carrier?.toLowerCase().includes(searchQuery) ||
        t.customer?.toLowerCase().includes(searchQuery) ||
        t.loadNumber?.toLowerCase().includes(searchQuery) ||
        'appointment'.includes(searchQuery)) {
      searchResults.push({ type: 'appointment', trailer: t });
    }
  });
...
function showSearchResultsModal() {
  ...
  const activeList = activeResults.map(r => {
    ...
    } else if (r.type === 'staging') {
      locationText = '⭐ Staging';
    } else if (r.type === 'queue') {
      locationText = `⏳ Queue (Door ${t.targetDoorNumber})`;
    } else if (r.type === 'appointment') {
      locationText = `📅 Appointment (${t.appointmentTime || 'TBD'})`;
...
async function bulkShipTrailers() {
  ...
  if (!await showConfirmModal({ title: 'Ship Trailers', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.75rem 0;">Mark <strong>${selectedTrailers.size}</strong> trailers as <span style="color: #8b5cf6;">SHIPPED</span>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">This will remove them from the active board.</p></div>`, type: 'info', confirmText: 'Ship', cancelText: 'Cancel' })) return;
...
async function bulkDeleteTrailers() {
  ...
  if (!await showConfirmModal({ title: 'Delete Trailers', html: `<div style="text-align: center;"><p style="color: var(--danger-color); margin: 0 0 0.75rem 0; font-weight: 500;">Permanently delete <strong>${selectedTrailers.size}</strong> trailers?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">This action cannot be undone.</p></div>`, type: 'danger', confirmText: 'Delete', cancelText: 'Cancel' })) return;
...
async function loadTrailerTimeline(trailerId) {
  ...
  try {
    ...
    if (timelineEvents.length === 0) {
      // Build timeline from current trailer state when history is empty
      const trailer = state.trailers.find(t => t.id === trailerId) ||
                      state.yardTrailers.find(t => t.id === trailerId);
      if (trailer) {
  ...
  return `
    <div class="timeline">
      ${events.map((e, i) => {
        ...
        const userDisplay = e.username ? `<span class="timeline-user">👤 ${e.username}</span>` : '';
        ...
        <div class="timeline-item ${isCurrent ? 'timeline-current' : ''} ${eventClass}">
          ...
          <div class="timeline-content">
            ...
            <span class="timeline-time">${new Date(e.timestamp).toLocaleString()} ${userDisplay}</span>
          </div>
        </div>
        `;
      }).join('')}
...
async function showAnalyticsModal() {
  ...
  modal.innerHTML = `
    <div class="modal-content modal-large">
      ...
      <div class="modal-body">
        <div class="analytics-facility-filter" id="analytics-facility-filter" style="margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border-color);">
          <label style="font-weight: 600; margin-right: 0.5rem;">Facility:</label>
          <select id="analytics-facility-select" style="padding: 0.5rem; border-radius: 4px; border: 1px solid var(--border-color); min-width: 200px;">
            <option value="current">Current Facility</option>
  ...
  const facilityFilterDiv = document.getElementById('analytics-facility-filter');
  const facilityInfo = document.getElementById('analytics-facility-info');

  if (authState.multiFacilityMode) {
    // Add individual facility options if available
    if (authState.availableFacilities?.length > 0) {
      authState.availableFacilities.forEach(facility => {
        const option = document.createElement('option');
        option.value = facility.id;
        option.textContent = facility.id;
        facilitySelect.appendChild(option);
      });
    }

    // Show facility info
    const currentFacility = authState.user?.currentFacility || 'Unknown';
    facilityInfo.textContent = `(Current: ${currentFacility})`;

    // Handle facility filter change
    facilitySelect.addEventListener('change', (e) => {
      currentFacilityFilter = e.target.value;
      loadAnalytics(currentPeriod);
      // Also reload violations if visible
      const violationsContainer = document.getElementById('violations-container');
      if (violationsContainer && !violationsContainer.classList.contains('hidden')) {
        loadViolations();
      }
      // Also reload patterns if visible
      const patternsContainer = document.getElementById('patterns-container');
      if (patternsContainer && !patternsContainer.classList.contains('hidden')) {
        loadPatterns();
      }
    });
  } else {
    // Hide facility filter in single facility mode
    facilityFilterDiv.style.display = 'none';
  }

  async function loadAnalytics(period) {
    try {
      let url = `/analytics?period=${period}`;
      // Add facility filter - 'current' means current facility (default)
      // 'all' means all facilities, specific ID means that facility
      if (currentFacilityFilter !== 'current') {
        url += `&facilities=${encodeURIComponent(currentFacilityFilter)}`;
      }
      analyticsData = await apiCall(url);
  ...
  async function loadViolations() {
    try {
      ...
      if (currentData.trailers.length === 0) {
        ...
        // Add click handlers to open edit modal (only for current facility)
...
async function fetchState() {
  ...
  try {
    ...
    // Re-apply saved sidebar layout after rendering
    applySidebarLayout();
...
function connectSSE() {
  ...
  if (!authState.token) {
    console.log('[SSE] Not authenticated, using fallback polling');
    startFallbackPolling();
    return;
  }

  // EventSource doesn't support custom headers, so pass token as query param
  const sseUrl = `/api/events?token=${encodeURIComponent(authState.token)}`;
  ...
  try {
    ...
    sseConnection.onmessage = (event) => {
      // Heartbeat messages (lines starting with :)
    ...
    // Listen for named events from server
    sseConnection.addEventListener('stateChange', (event) => {
      try {
        const data = JSON.parse(event.data);
        console.log('[SSE] State change:', data);
        // Fetch fresh state to sync with server
        fetchState();
      } catch (err) {
    ...
    sseConnection.onerror = (err) => {
      ...
      if (sseReconnectAttempts < MAX_SSE_RECONNECT_ATTEMPTS) {
        ...
        const delay = SSE_RECONNECT_DELAY * Math.min(sseReconnectAttempts, 5);
...
function startFallbackPolling() {
  // Fall back to traditional polling if SSE fails
  if (!pollingInterval) {
...
function renderDoors() {
  ...
  grid.querySelectorAll('.trailer-card[draggable]').forEach(el => {
    ...
    el.addEventListener('click', (e) => {
      // Pass event to selection handler for Ctrl/Cmd key detection
  ...
  grid.querySelectorAll('.delete-trailer-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (!await showConfirmModal({ title: 'Delete Trailer', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.5rem 0;">Delete trailer <strong>${displayName}</strong>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">Carrier: ${decodeHtml(trailer.carrier)}</p></div>`, type: 'danger', confirmText: 'Delete', cancelText: 'Cancel' })) {
        return;
      }
...
function renderQueue() {
  ...
  container.innerHTML = sortedQueue.map((t, index) => {
    ...
    if (t.appointmentTime) {
      // Format HH:MM time value to localized display string
...
function renderYardSlots() {
  ...
  list.querySelectorAll('.delete-trailer-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (!await showConfirmModal({ title: 'Delete Trailer', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.5rem 0;">Delete trailer <strong>${trailer.number}</strong>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">Carrier: ${trailer.carrier}</p></div>`, type: 'danger', confirmText: 'Delete', cancelText: 'Cancel' })) {
        return;
      }
...
function openEditYardSlotModal(slot) {
  ...
  document.getElementById('btn-delete-slot')?.addEventListener('click', async () => {
    if (!await showConfirmModal({ title: 'Delete Yard Slot', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.75rem 0;">Delete yard slot <strong>${slot.number}</strong>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">Any trailers in this slot will be moved to unassigned yard.</p></div>`, type: 'warning', confirmText: 'Delete', cancelText: 'Cancel' })) return;
    try {
...
function renderUnassignedYard() {
  ...
  if (unassignedCount) unassignedCount.textContent = `${activeTrailers.length} trailer${activeTrailers.length !== 1 ? 's' : ''} in unassigned yard`;
  if (!yard) return;
...
function updateUnassignedCount() {
  ...
  const count = (state.yardTrailers || []).filter(t => !t.yardSlotId).length;
...
// Close all dropdown menus (user menu, etc.)
function closeAllDropdowns() {
  const userDropdown = document.getElementById('user-dropdown');
  if (userDropdown) {
 ...
 * @param {Object} options - { title, message, type, confirmText, cancelText, requireInput, inputPlaceholder }
 * @returns {Promise<string|boolean>} - Resolves with input value or true, rejects with false
 */
function showConfirmModal(options = {}) {
  const {
    title = 'Confirm',
    message = '',
    html = '', // Alternative: pass pre-formatted HTML content
  ...
  return new Promise((resolve) => {
    ...
    const modal = document.createElement('div');
    modal.className = 'modal active confirm-modal';
    modal.id = 'modal-custom-confirm';

    // Icon based on type
    let icon = '⚠️';
    let headerColor = '#f59e0b'; // amber
    if (type === 'danger') {
      icon = '🗑️';
    ...
    // Build content - restructured with clear sections
    let content = `
      <div class="modal-content" style="max-width: 400px; padding: 0; display: flex; flex-direction: column;">
        <!-- Header: Icon + Title -->
        <div style="padding: 2rem 1.5rem 1rem; text-align: center;">
          <div style="font-size: 2rem; line-height: 1; margin-bottom: 0.75rem;">${icon}</div>
          <h3 style="color: ${headerColor}; font-size: 1.125rem; margin: 0; font-weight: 600;">${title}</h3>
        </div>

        <!-- Body: Content -->
        <div style="padding: 0 1.5rem 1.25rem; text-align: center;">
          ${html ? html : `<p style="color: var(--text-secondary); line-height: 1.6; margin: 0; font-size: 0.9375rem;">${message}</p>`}
          ${requireInput ? `
            <input type="text" id="confirm-input" placeholder="${inputPlaceholder}"
              style="width: 100%; margin-top: 1.25rem; padding: 0.75rem; border: 1px solid var(--border-color);
...
function setupModals() {
  ...
  // Backdrop click - skip setup-modal (it's non-dismissible)
  document.querySelectorAll('.modal').forEach(modal => {
    if (modal.id === 'setup-modal') return; // Setup modal handles its own clicks
    modal.addEventListener('click', (e) => {
  ...
  document.getElementById('form-create-trailer')?.addEventListener('submit', async (e) => {
    ...
    try {
      if (targetStaging) {
        // Add directly to staging
        await addToStaging(data);
        showToast(`✅ ${data.carrier} trailer added to Staging!`, 'success');
      } else {
...
async function loadHistory(search = '', append = false) {
  ...
  try {
    ...
    const newEntries = data.entries.map(h => {
      ...
      const userDisplay = h.username ? `<span class="history-user">👤 ${h.username}</span>` : '';
      ...
      <div class="history-item" data-trailer-id="${h.trailerId || ''}" title="Double-click to edit trailer">
        <div class="history-item-header">
          ...
          <span class="history-time">${new Date(h.timestamp).toLocaleString()} ${userDisplay}</span>
        </div>
  ...
  try {
    ...
    list.querySelectorAll('.btn-delete-shipped').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!await showConfirmModal({ title: 'Delete Shipped Order', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.5rem 0;">Delete shipped order for <strong>${shipped.carrier}</strong>?</p>${shipped.number ? `<p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">Trailer: ${shipped.number}</p>` : ''}</div>`, type: 'danger', confirmText: 'Delete', cancelText: 'Cancel' })) return;
...
// Load and display unassigned yard trailers (Lost and Found)
async function loadUnassignedYard(search = '') {
  const list = document.getElementById('unassigned-list');
  const emptyMsg = document.getElementById('unassigned-empty');
  ...
  try {
    ...
    let trailers = state.yardTrailers || [];

    // Filter out trailers that are assigned to yard slots (truly unassigned only)
    trailers = trailers.filter(t => !t.yardSlotId);
    ...
    // Sort by created date (oldest first - they've been waiting longest)
    trailers.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));

    if (countEl) countEl.textContent = `${trailers.length} trailer${trailers.length !== 1 ? 's' : ''} in unassigned yard`;
    ...
    list.innerHTML = trailers.map(t => {
      ...
      const dwellHours = getDwellTimeHours(t.createdAt);
      const dwellClass = getDwellTimeClass(t.createdAt);

      return `
        <div class="unassigned-item" data-trailer-id="${t.id}">
          <div class="unassigned-item-header">
            <span class="unassigned-carrier">${t.carrier || 'Unknown'}</span>
            <div style="display: flex; gap: 0.5rem; align-items: center;">
              ${editMode ? `
              <button class="btn-move-staging" data-trailer-id="${t.id}" title="Move to staging" ${state.staging ? 'disabled' : ''}>⭐</button>
              <button class="btn-ship-unassigned" data-trailer-id="${t.id}" title="Mark as shipped">📦</button>
              <button class="btn-delete-unassigned" data-trailer-id="${t.id}" title="Delete trailer">🗑️</button>` : ''}
              <span class="unassigned-status">UNASSIGNED</span>
              <span class="unassigned-date">Added: ${createdDate}</span>
            </div>
          </div>
          <div class="unassigned-details">
            ${t.number ? `<div class="unassigned-detail"><span class="unassigned-detail-label">Trailer:</span> ${t.number}</div>` : ''}
            ${t.customer ? `<div class="unassigned-detail"><span class="unassigned-detail-label">Customer:</span> ${t.customer}</div>` : ''}
            ${t.loadNumber ? `<div class="unassigned-detail"><span class="unassigned-detail-label">Load:</span> ${t.loadNumber}</div>` : ''}
            ${dwellHours !== null ? `<div class="unassigned-detail"><span class="unassigned-detail-label">Dwell Time:</span> <span class="${dwellClass}">${dwellHours}h</span></div>` : ''}
            ${t.driverName ? `<div class="unassigned-detail"><span class="unassigned-detail-label">Driver:</span> ${t.driverName}</div>` : ''}
            ${t.driverPhone ? `<div class="unassigned-detail"><span class="unassigned-detail-label">Phone:</span> ${t.driverPhone}</div>` : ''}
            ${t.contents ? `<div class="unassigned-detail"><span class="unassigned-detail-label">Notes:</span> ${t.contents}</div>` : ''}
          </div>
        </div>
      `;
    }).join('');

    // Add double-click handlers to edit trailer
    ...
    // Add move to staging handlers
    list.querySelectorAll('.btn-move-staging').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const trailerId = btn.dataset.trailerId;
        if (!trailerId) return;
        ...
        if (state.staging) {
          showToast('Staging area is already occupied', 'error');
          return;
        }
        ...
        if (!await showConfirmModal({ title: 'Move to Staging', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.5rem 0;">Move trailer <strong>${trailer.number || trailer.carrier}</strong> to Staging?</p><p style="color: var(--accent-secondary); margin: 0; font-size: 0.875rem;">⭐ Priority staging area</p></div>`, type: 'info', confirmText: 'Move', cancelText: 'Cancel' })) return;

        try {
          await moveTrailerToStaging(trailerId);
          showToast('Trailer moved to staging', 'success');
          loadUnassignedYard(search);
          renderStaging();
        } catch (err) {
          showToast(err.message || 'Failed to move trailer', 'error');
        }
      });
    });

    // Add ship handlers
    list.querySelectorAll('.btn-ship-unassigned').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const trailerId = btn.dataset.trailerId;
        if (!trailerId) return;
        ...
        if (!await showConfirmModal({ title: 'Mark as Shipped', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.5rem 0;">Mark trailer <strong>${trailer.number || trailer.carrier}</strong> as shipped?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">This will archive the trailer record.</p></div>`, type: 'info', confirmText: 'Ship', cancelText: 'Cancel' })) return;
        ...
        try {
          ...
          showToast(err.message || 'Failed to ship trailer', 'error');
        }
      });
    });

    // Add delete handlers
    list.querySelectorAll('.btn-delete-unassigned').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const trailerId = btn.dataset.trailerId;
        if (!trailerId) return;
        ...
        if (!await showConfirmModal({ title: 'Delete Trailer', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.75rem 0;">Delete trailer <strong>${trailer.number || trailer.carrier}</strong>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">This action cannot be undone.</p></div>`, type: 'danger', confirmText: 'Delete', cancelText: 'Cancel' })) return;
    ...
    console.error('Unassigned yard load error:', error);
    list.innerHTML = '<div class="unassigned-empty">Failed to load unassigned yard</div>';
  }
}

// Move trailer from unassigned yard to staging
async function moveTrailerToStaging(trailerId) {
  // Use the staging POST endpoint
  const trailer = state.yardTrailers.find(t => t.id === trailerId);
  if (!trailer) throw new Error('Trailer not found');

  const response = await apiCall('/staging', 'POST', {
    number: trailer.number,
    carrier: trailer.carrier,
    status: trailer.status || 'loaded',
    customer: trailer.customer,
    loadNumber: trailer.loadNumber,
    driverName: trailer.driverName,
    driverPhone: trailer.driverPhone,
    contents: trailer.contents,
    isLive: trailer.isLive || false,
    sourceId: trailerId // Tell backend to remove from yardTrailers
  });

  return response;
}

// Add event listeners for unassigned yard search and refresh
function setupUnassignedYardEvents() {
  const searchInput = document.getElementById('unassigned-search');
  const clearBtn = document.getElementById('unassigned-clear');
  const refreshBtn = document.getElementById('unassigned-refresh');

  let searchDebounce;
  searchInput?.addEventListener('input', (e) => {
    clearTimeout(searchDebounce);
    searchDebounce = setTimeout(() => loadUnassignedYard(e.target.value), 300);
  });
  ...
  refreshBtn?.addEventListener('click', () => {
    loadUnassignedYard(searchInput?.value || '');
    showToast('Unassigned yard refreshed', 'success');
  });
}
...
function renderCarriersList(search = '') {
  ...
  document.querySelectorAll('.btn-delete-carrier').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (!await showConfirmModal({ title: 'Delete Carrier', html: `<p style="color: var(--text-secondary); margin: 0;">Delete carrier "<strong>${carrier.name}</strong>"?</p>`, type: 'warning', confirmText: 'Delete', cancelText: 'Cancel' })) return;
...
function sanitizeInput(value) {
  // UX-level sanitizer: strips HTML tags for display safety
  // Backend performs the security sanitization
  if (!value) return '';
...
function openDoorEditor(doorId) {
  ...
  document.getElementById('btn-delete-door')?.addEventListener('click', async () => {
    if (!await showConfirmModal({ title: 'Delete Door', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.75rem 0;">Delete Door <strong>${door.number}</strong>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">Any trailer inside will be moved to the yard.</p></div>`, type: 'danger', confirmText: 'Delete', cancelText: 'Cancel' })) {
      return;
    }
...
function openTrailerEditModal(trailerId) {
  ...
  // Check where the trailer actually is
  const inStaging = state.staging?.id === trailerId;
  const inQueue = state.queuedTrailers?.find(t => t.id === trailerId);
  const inAppt = state.appointmentQueue?.find(t => t.id === trailerId);

  const location = trailer.doorNumber ? `Door ${trailer.doorNumber}` :
                   trailer.yardSlotNumber ? `Yard Spot ${trailer.yardSlotNumber}` :
                   inStaging ? '⭐ Staging' :
                   inQueue ? `⏳ Queue (Door ${inQueue.targetDoorNumber})` :
                   inAppt ? `📅 Appointment (${inAppt.appointmentTime || 'TBD'})` :
                   'Unassigned Yard';
  ...
  document.getElementById('btn-delete-trailer-edit')?.addEventListener('click', async () => {
    ...
    if (!await showConfirmModal({ title: 'Delete Trailer', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.75rem 0;">Delete trailer <strong>${displayName}</strong>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">This action cannot be undone.</p></div>`, type: 'danger', confirmText: 'Delete', cancelText: 'Cancel' })) {
      return;
    }
    try {
      await deleteTrailer(trailerId);
  ...
  document.getElementById('btn-ship-trailer')?.addEventListener('click', async () => {
    if (!await showConfirmModal({ title: 'Ship Trailer', html: `<div style="text-align: left;"><p style="color: var(--text-secondary); margin: 0 0 0.75rem 0;">Mark trailer <strong>${trailer.number || trailer.carrier}</strong> as shipped?</p><ul style="margin: 0 0 1rem 0; padding-left: 1.25rem; color: var(--text-secondary); line-height: 1.6; font-size: 0.875rem;"><li>Removed from active view</li><li>Preserved in history</li><li>Searchable by number/load</li><li>Reusable if it returns</li></ul><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">This action cannot be undone.</p></div>`, type: 'info', confirmText: 'Ship', cancelText: 'Cancel' })) {
      return;
    }
    try {
      await shipTrailer(trailerId);
  ...
  // Move to Staging handler (for unassigned yard trailers)
  document.getElementById('btn-move-to-staging')?.addEventListener('click', async () => {
    if (state.staging) {
      showToast('Staging area is already occupied', 'error');
      return;
    }

    if (!await showConfirmModal({ title: 'Move to Staging', html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.5rem 0;">Move trailer <strong>${trailer.number || trailer.carrier}</strong> to Staging?</p><p style="color: var(--accent-secondary); margin: 0; font-size: 0.875rem;">⭐ Priority staging area</p></div>`, type: 'info', confirmText: 'Move', cancelText: 'Cancel' })) {
      return;
    }

    try {
      // Move trailer data to staging
      const response = await apiCall('/staging', 'POST', {
        number: trailer.number,
        carrier: trailer.carrier,
        status: trailer.status || 'loaded',
      ...
      if (response.success) {
        ...
        modal.remove();
        fetchState();
        // Refresh unassigned yard if it's open
        if (document.getElementById('modal-unassigned')?.classList.contains('active')) {
          loadUnassignedYard(document.getElementById('unassigned-search')?.value || '');
        }
      } else {
        throw new Error(response.error || 'Failed to move trailer');
      }
    } catch (error) {
      showToast(error.message || 'Failed to move to staging', 'error');
    }
  });
...
function openTimePicker(targetInputId) {
  ...
  // Generate time slots from 6AM to 10PM in 30-minute increments
  const slots = [];
  for (let h = 6; h <= 22; h++) {
...
async function checkSetupStatus() {
  try {
    ...
    return { setupNeeded: false, usersExist: true, hasAdmin: true };
  }
}

// Show setup modal (non-dismissible during initial setup, closable when adding facilities)
function showSetupModal(isNewFacility = false) {
  ...
  if (isNewFacility) {
    if (header) header.innerHTML = '🏭 Create New Facility';
    if (subtitle) subtitle.textContent = 'Configure a new facility to add to the system';
  } else {
    if (header) header.innerHTML = '🏭 Welcome to Warehouse Dock Board';
    if (subtitle) subtitle.textContent = 'Configure your facility to get started';
  }

  // Show facility name/ID section in multi-facility mode or initial setup
  const facilityIdSection = document.getElementById('setup-facility-name-section');
  if (facilityIdSection) {
    // Always show name field; only show ID field in multi-facility mode
    facilityIdSection.classList.remove('hidden');

    // Show/hide the ID field based on mode
    const facilityIdLabel = facilityIdSection.querySelector('label[for="setup-facility-id"]');
    const facilityIdInput = document.getElementById('setup-facility-id');
    if (facilityIdLabel && facilityIdInput) {
      if (authState.multiFacilityMode) {
        facilityIdLabel.classList.remove('hidden');
        facilityIdInput.classList.remove('hidden');
      } else {
        facilityIdLabel.classList.add('hidden');
        facilityIdInput.classList.add('hidden');
      }
    }

    // Clear values for new facility creation
    if (facilityIdInput) facilityIdInput.value = '';
    const facilityNameInput = document.getElementById('setup-facility-name');
    if (facilityNameInput) facilityNameInput.value = '';
  } else {
    facilityIdSection.classList.add('hidden');
  }

  // Reset form values to defaults
  const doorsInput = document.getElementById('setup-doors');
  const yardSlotsInput = document.getElementById('setup-yard-slots');
  const dumpstersInput = document.getElementById('setup-dumpsters');
  const rampsInput = document.getElementById('setup-ramps');

  if (doorsInput) doorsInput.value = '57';
  if (yardSlotsInput) yardSlotsInput.value = '30';
  if (dumpstersInput) dumpstersInput.value = '0';
  if (rampsInput) rampsInput.value = '0';

  // Allow closing modal when adding a new facility (not during initial setup)
  const allowClose = isNewFacility && authState.isAuthenticated;

  // Prevent backdrop click from closing modal (unless adding new facility)
  modal.addEventListener('click', (e) => {
    if (e.target === modal && !allowClose) {
      e.stopPropagation();
      // Don't close - setup is required
    } else if (e.target === modal && allowClose) {
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          // Get the new facility ID from the response
          const newFacilityId = data.facility?.id;
          if (newFacilityId) {
            // Switch to the new facility via API to get updated token
            try {
              const switchRes = await fetch('/api/auth/switch-facility', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${authState.token}`
              ...
              if (switchRes.ok) {
                ...
                // Update token and auth state with new facility
                if (switchData.token) {
                  authState.token = switchData.token;
                  localStorage.setItem('dockboard_token', switchData.token);
                }
                if (switchData.user) {
                  authState.user = switchData.user;
                }
              }
            } catch (e) {
...
// Show "waiting for admin" modal for non-admin users when facility needs setup
function showWaitingForAdminModal() {
  const modal = document.createElement('div');
  modal.id = 'modal-waiting-admin';
  modal.className = 'modal active';
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 450px; text-align: center;">
      <div class="modal-header" style="border-bottom: none; padding-bottom: 0;">
        ...
        <h3 style="color: #f59e0b; font-size: 1.5rem; margin: 0;">Waiting for Admin</h3>
      </div>
      <div class="modal-body" style="padding: 1.5rem 2rem;">
        <p style="color: var(--text-secondary); line-height: 1.6; margin-bottom: 1rem;">
          The facility has not been created yet.
        </p>
        <p style="color: var(--text-secondary); line-height: 1.6; margin-bottom: 1rem;">
          Only an administrator can create the facility layout.<br>
          Please wait for your admin to complete the setup.
        </p>
        <div style="background: var(--bg-tertiary); padding: 1rem; border-radius: var(--radius-md); margin-top: 1rem;">
          <p style="color: var(--text-muted); font-size: 0.875rem; margin: 0;">
            Your account: <strong>${authState.user?.username || 'Unknown'}</strong><br>
            Role: <strong>${authState.user?.role || 'Unknown'}</strong>
          </p>
        </div>
      </div>
      <div class="modal-actions" style="padding: 0 2rem 1.5rem; gap: 1rem; border-top: none;">
        <button id="btn-waiting-refresh" class="btn btn-primary" style="flex: 1;">
          🔄 Refresh
        </button>
        <button id="btn-waiting-logout" class="btn btn-secondary" style="flex: 1;">
          Logout
        </button>
      </div>
    </div>
  ...
  // Refresh button
  document.getElementById('btn-waiting-refresh')?.addEventListener('click', () => {
    window.location.reload();
  });
...
// Show "Waiting for Admin" modal when facility isn't set up yet
function showWaitingForAdminModal() {
  // Remove any existing waiting modal
  const existing = document.getElementById('modal-waiting-admin');
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 450px; text-align: center;">
      <div class="modal-header" style="border-bottom: none; padding-bottom: 0;">
        <div style="font-size: 3rem; margin-bottom: 0.5rem;">⏳</div>
        <h3 style="color: #f59e0b; font-size: 1.5rem; margin: 0;">Facility Not Ready</h3>
      </div>
      <div class="modal-body" style="padding: 1.5rem 2rem;">
        <div style="color: var(--text-secondary); line-height: 1.6; font-size: 0.95rem;">
          <p>This facility hasn't been configured yet.</p>
          <p style="margin-top: 1rem;">Please wait for an <strong>administrator</strong> to set up the facility.</p>
          <p style="margin-top: 1rem; font-size: 0.85rem; color: var(--text-muted);">
            Once the admin creates the facility layout, refresh this page to access the application.
          </p>
        </div>
      </div>
      <div class="modal-actions" style="padding: 0 2rem 1.5rem; gap: 1rem; border-top: none;">
        <button id="btn-refresh" class="btn btn-primary" style="flex: 1;">🔄 Refresh</button>
        <button id="btn-logout-wait" class="btn btn-secondary" style="flex: 1;">Logout</button>
      </div>
    </div>
  `;
  ...
  // Logout button
  document.getElementById('btn-logout-wait')?.addEventListener('click', () => {
    logout();
    modal.remove();
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Set up edit mode toggle with auth check - admin only
  document.getElementById('btn-edit-mode')?.addEventListener('click', async () => {
    ...
    if (authState.user?.role !== 'admin') { showToast('Admin access required', 'error'); return; }
  ...
  // Generate demo data button (bootstrap admin only)
  document.getElementById('btn-generate-demo')?.addEventListener('click', async () => {
    if (!requireAuth()) return;
    const confirmed = await showConfirmModal({
      title: 'Generate Demo Data',
      message: 'This will clear all current trailers and generate demo data based on your facility size. Continue?',
      type: 'warning',
    ...
    try {
      ...
      const res = await fetch('/api/demo/generate', {
        ...
        body: JSON.stringify({ count: 0 }) // 0 = auto-scale based on facility
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to generate demo data');
      showToast('Demo data generated successfully', 'success');
      fetchState();
    } catch (error) {
      showToast('Failed to generate demo data: ' + error.message, 'error');
    }
  });

  // Reset facility button (edit mode only) - Clear all data and restart
  document.getElementById('btn-reset-facility')?.addEventListener('click', async () => {
    if (!requireAuth()) return;

    // Step 1: First confirmation
    const step1 = await showConfirmModal({
      title: 'Reset Facility',
      html: '<div style="text-align: left;"><p style="color: var(--danger-color); margin: 0 0 0.75rem 0; font-weight: 500;">This will DELETE ALL DATA:</p><ul style="margin: 0 0 1rem 0; padding-left: 1.25rem; color: var(--text-secondary); line-height: 1.7;"><li>All trailers (dock, yard, staging, queues)</li><li>All doors and yard slots</li><li>All history and analytics</li><li>All settings</li></ul><p style="color: var(--danger-color); margin: 0 0 0.5rem 0; font-weight: 500;">This action CANNOT be undone.</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">Are you absolutely sure?</p></div>',
      type: 'danger',
    ...
    const typed = await showConfirmModal({
      title: 'Final Warning',
      html: '<div style="text-align: left;"><p style="color: var(--text-secondary); margin: 0 0 0.75rem 0;">Type <strong>"delete all data"</strong> to confirm permanent deletion of ALL facility data:</p><ul style="margin: 0 0 1rem 0; padding-left: 1.25rem; color: var(--text-muted); font-size: 0.875rem; line-height: 1.6;"><li>All trailers</li><li>All doors and yard configuration</li><li>All history and analytics</li></ul><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">You will be returned to the setup wizard.</p></div>',
      type: 'danger',
      confirmText: 'Delete All Data',
      cancelText: 'Cancel',
      requireInput: true,
      inputPlaceholder: 'Type "delete all data" here'
    });

    if (typed?.toLowerCase().trim() !== 'delete all data') {
    ...
    try {
      ...
      const data = await res.json();
      showToast('✅ Facility reset. Recreating...', 'success');

      // Update auth state to remove current facility
      if (authState.user) {
        authState.user.currentFacility = null;
        authState.user.homeFacility = null;
      }

      // Update UI to reflect we're not in a facility
      updateAuthUI();

      // Show setup modal immediately to recreate the facility
      setTimeout(() => {
        showSetupModal(true);
      }, 500);
  ...
  document.getElementById('btn-user-settings')?.addEventListener('click', () => {
    userDropdown?.classList.add('hidden');
    if (requireAuth()) openSettingsModal();
  });

  // Change Password - Not available for bootstrap admin
  document.getElementById('btn-change-password')?.addEventListener('click', () => {
    ...
    if (authState.user?.isBootstrap) {
      showToast('Bootstrap admin password must be changed via environment variable', 'error');
      return;
    }
  ...
  document.getElementById('btn-manage-users')?.addEventListener('click', () => {
    userDropdown?.classList.add('hidden');
    if (requireAuth()) openUsersManagementModal();
  });

  document.getElementById('btn-manage-facilities')?.addEventListener('click', () => {
    userDropdown?.classList.add('hidden');
    if (requireAuth()) openFacilitiesManagementModal();
  });

  // Switch Facility button - opens facility selector modal
  document.getElementById('btn-switch-facility')?.addEventListener('click', () => {
    console.log('[SwitchFacilityBtn] Clicked');
    userDropdown?.classList.add('hidden');
    const isAuth = requireAuth();
    console.log('[SwitchFacilityBtn] requireAuth:', isAuth, 'availableFacilities:', authState.availableFacilities?.length);
    if (isAuth && authState.availableFacilities?.length > 0) {
      showFacilitySelectorModal(authState.availableFacilities, true); // true = switching mode
    }
  });
  ...
  async function openUsersManagementModal() {
    console.log('[Users] Opening user management modal');
    ...
    console.log('[Users] Modal opened, rendering users list');
  ...
  // Render users list in the modal
  async function renderUsersList() {
    ...
    try {
      ...
      if (users.length === 0) {
        container.innerHTML = '<p style="color: var(--text-secondary); text-align: center; padding: 2rem;">No users found.</p>';
        return;
      }

      container.innerHTML = users.map(user => `
        <div class="user-item" style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem; border-bottom: 1px solid var(--border-color);">
          <div style="display: flex; flex-direction: column; gap: 0.25rem;">
            <span style="font-weight: 600;">${user.username}${user.passwordResetRequired ? ' 🔐' : ''}</span>
            <span style="font-size: 0.75rem; color: var(--text-secondary);">Role: ${user.role}${user.lastLogin ? ` | Last login: ${new Date(user.lastLogin).toLocaleString()}` : ''}${user.passwordResetRequired ? ' | Reset required' : ''}</span>
          </div>
          <div style="display: flex; gap: 0.5rem;">
            ${user.id !== authState.user?.userId && !user.isBootstrap ? `
              <button class="btn btn-small btn-warning btn-reset-password" data-user-id="${user.id}" data-username="${user.username}" title="Require password reset on next login">Reset Password</button>
              <button class="btn btn-small btn-danger btn-delete-user" data-user-id="${user.id}">Delete</button>
            ` : user.isBootstrap ? '<span style="font-size: 0.75rem; color: var(--text-muted);" title="Bootstrap admin password must be changed via environment variable">Bootstrap Admin</span>' : '<span style="font-size: 0.75rem; color: var(--text-muted);">You</span>'}
          </div>
        </div>
      `).join('');
      ...
      container.querySelectorAll('.btn-reset-password').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          const confirmed = await showConfirmModal({
            ...
            html: `<div style="text-align: center;"><p style="color: var(--text-secondary); margin: 0 0 0.5rem 0;">Require password reset for <strong>"${username}"</strong>?</p><p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">They will be forced to set a new password on their next login.</p></div>`,
            type: 'warning',
          ...
          try {
            const result = await apiCall(`/users/${userId}/reset-password`, 'POST');
            ...
            if (tempPassword) {
              ...
              await showConfirmModal({
                ...
                html: `<div style="text-align: center;">
                  <p style="color: var(--text-secondary); margin: 0 0 1rem 0;">Give this temporary password to <strong>${username}</strong>:</p>
                  <div style="background: var(--bg-primary); padding: 1rem; border-radius: var(--radius-md); margin: 1rem 0; font-family: monospace; font-size: 1.5rem; letter-spacing: 0.1em; color: var(--accent-primary); font-weight: bold;">${tempPassword}</div>
                  <p style="color: var(--text-muted); margin: 0; font-size: 0.875rem;">The user must enter this password on login, then set their own new password.</p>
                </div>`,
                type: 'info',
                confirmText: 'Done',
      ...
      container.querySelectorAll('.btn-delete-user').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          const confirmed = await showConfirmModal({
            title: 'Delete User',
            html: `<p style="color: var(--text-secondary); margin: 0;">Delete user <strong>"${user.username}"</strong>?</p>`,
            type: 'danger',
            confirmText: 'Delete',
            cancelText: 'Cancel'
          });
      ...
      container.innerHTML = '<p style="color: #ef4444; text-align: center; padding: 2rem;">Failed to load users.</p>';
    }
  }
  ...
  function setupSidebarResizing() {
    ...
    handles.forEach(handle => {
      handle.addEventListener('mousedown', (e) => {
        ...
        const viewportEdgeThreshold = 30; // pixels from edge to trigger scroll
        ...
        const onMouseMove = (moveEvent) => {
          ...
          const isNearBottomEdge = moveEvent.clientY >= viewportHeight - viewportEdgeThreshold;

          // Auto-scroll page when near viewport edge during resize (Excel-style continuous scroll)
          if (isBottomHandle && isNearBottomEdge) {
            if (!autoScrollInterval) {
              autoScrollInterval = setInterval(() => {
                window.scrollBy(0, 8);
                currentHeight += 8; // Increase height as page scrolls
                sidebar.style.height = `${Math.max(400, currentHeight)}px`;
                sidebar.style.maxHeight = 'none';
              }, 16); // ~60fps continuous scroll
            }
          } else {
            stopAutoScroll();
          }
          ...
          if (isBottomHandle) {
            currentHeight = Math.max(400, startHeight + deltaY);
        ...
        const onMouseUp = () => {
          ...
          if (sidebarEl && stagingEl && queueEl) {
            ...
            // Save to user settings if authenticated, otherwise fall back to global settings
            const savePath = (authState.isAuthenticated && authState.token) ? '/user/settings' : '/settings';
            apiCall(savePath, 'POST', { sidebarLayout: layout })
...
// ============================================
// FACILITIES MANAGEMENT MODAL (Bootstrap Admin only)
// ============================================
async function openFacilitiesManagementModal() {
  console.log('[Facilities] Opening facility management modal');

  // Create modal if it doesn't exist
  let modal = document.getElementById('modal-facilities');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'modal-facilities';
    modal.className = 'modal';
    modal.innerHTML = `
      <div class="modal-content" style="max-width: 600px;">
        ...
        <div class="modal-footer" style="border-top: 1px solid var(--border); padding: 1rem;">
          <button id="btn-create-facility" class="btn btn-primary">➕ Create New Facility</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
...
async function renderFacilitiesList() {
  ...
  try {
    const data = await apiCall('/facilities');
    const facilities = data.facilities || [];

    if (facilities.length === 0) {
      container.innerHTML = '<p style="color: var(--text-secondary); text-align: center; padding: 2rem;">No facilities found.</p>';
      return;
    }

    container.innerHTML = facilities.map(facility => `
      <div class="facility-item" style="padding: 1rem; border: 1px solid var(--border); border-radius: 0.5rem; margin-bottom: 0.5rem; ${facility.id === authState.user?.currentFacility ? 'background: var(--primary-light);' : ''}">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <div>
            <div style="font-weight: 600; font-size: 1.1rem;">${escapeHtml(facility.id)}</div>
            
            <div style="font-size: 0.75rem; color: var(--text-secondary); margin-top: 0.25rem;">
              ${facility.stats?.userCount || 0} users · ${facility.stats?.doorCount || 0} doors · ${facility.stats?.trailerCount || 0} trailers
            </div>
          </div>
          <div style="display: flex; gap: 0.5rem;">
...
function showCreateFacilityModal() {
  const modal = document.createElement('div');
  modal.id = 'modal-create-facility';
  modal.className = 'modal active';
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 500px;">
      <div class="modal-header">
      ...
      <form id="create-facility-form" style="padding: 1rem;">
        <div style="margin-bottom: 1rem;">
          ...
          <input type="text" id="facility-name" required style="width: 100%; padding: 0.5rem; border: 1px solid var(--border); border-radius: 0.25rem;" placeholder="e.g., Warehouse B">
        </div>
        <div style="margin-bottom: 1rem;">
          <label style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Description</label>
          <textarea id="facility-description" rows="2" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border); border-radius: 0.25rem;" placeholder="Optional description"></textarea>
        </div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
          <div>
            <label style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Doors</label>
            <input type="number" id="facility-doors" value="57" min="1" max="200" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border); border-radius: 0.25rem;">
          </div>
          <div>
            <label style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Yard Slots</label>
        ...
        <div style="margin-bottom: 1rem; padding: 0.75rem; background: var(--background-light); border-radius: 0.25rem;">
          ...
          <input type="text" id="facility-admin-username" required style="width: 100%; padding: 0.5rem; border: 1px solid var(--border); border-radius: 0.25rem; margin-bottom: 0.5rem;" placeholder="Admin username">
          <input type="password" id="facility-admin-password" required style="width: 100%; padding: 0.5rem; border: 1px solid var(--border); border-radius: 0.25rem;" placeholder="Admin password">
        </div>
  ...
  document.getElementById('create-facility-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      closeModal('modal-create-facility');
