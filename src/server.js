...
app.use(cors());
app.use(express.json({ limit: '50mb' })); // Allow large archive uploads
...
function validateArchiveData(data) {
  ...
  const requiredArrays = ['doors', 'trailers', 'carriers', 'yardTrailers', 'yardSlots'];
  ...
  for (const door of data.doors) {
    if (typeof door !== 'object') return { valid: false, error: 'Invalid door entry' };
    if (typeof door.id !== 'string' || !door.id.match(/^[\w\-]+$/)) {
      return { valid: false, error: 'Invalid door ID' };
    }
    if (door.number !== null && (typeof door.number !== 'number' || door.number < 0)) {
      return { valid: false, error: 'Invalid door number' };
    }
    if (typeof door.order !== 'number') return { valid: false, error: 'Invalid door order' };
    ...
    for (const [key, value] of Object.entries(door)) {
      if (typeof value === 'string' && value.match(/[<>{}]|function\s*\(|\bexec\b|\beval\b|\brequire\b|\bimport\b/)) {
  ...
  for (const trailer of data.trailers) {
    if (typeof trailer !== 'object') return { valid: false, error: 'Invalid trailer entry' };
    if (typeof trailer.id !== 'string' || !trailer.id.match(/^[\w\-]+$/)) {
      return { valid: false, error: 'Invalid trailer ID' };
    }
    if (trailer.number != null && typeof trailer.number !== 'string' && typeof trailer.number !== 'number') {
  ...
  for (const slot of data.yardSlots) {
    if (typeof slot !== 'object') return { valid: false, error: 'Invalid yard slot' };
    if (typeof slot.id !== 'string') return { valid: false, error: 'Invalid yard slot ID' };
    if (typeof slot.number !== 'number') return { valid: false, error: 'Invalid yard slot number' };
  }
...
function sanitizeArchiveData(data) {
  const sanitized = {
    ...
    queuedTrailers: Array.isArray(data.queuedTrailers) ? data.queuedTrailers : [],
    appointmentQueue: Array.isArray(data.appointmentQueue) ? data.appointmentQueue : [],
    shippedTrailers: Array.isArray(data.shippedTrailers) ? data.shippedTrailers : []
  };

  // Sanitize doors
  for (const door of data.doors) {
    sanitized.doors.push({
      id: String(door.id).replace(/[^\w\-]/g, '').slice(0, 50),
      number: door.number === null ? null : Math.floor(Number(door.number)),
      order: Math.floor(Number(door.order)) || 0,
      trailerId: door.trailerId ? String(door.trailerId).replace(/[^\w\-]/g, '').slice(0, 50) : null,
      status: ['empty', 'occupied', 'out-of-service'].includes(door.status) ? door.status : 'empty',
      inService: Boolean(door.inService),
      type: ['normal', 'blank', 'out-of-service'].includes(door.type) ? door.type : 'normal',
      labelText: door.labelText ? String(door.labelText).replace(/[<>]/g, '').slice(0, 100) : undefined
    });
  }
  ...
  for (const trailer of data.trailers) {
    sanitized.trailers.push({
      id: String(trailer.id).replace(/[^\w\-]/g, '').slice(0, 50),
      ...
      status: ['empty', 'loaded', 'shipped'].includes(trailer.status) ? trailer.status : 'empty',
      doorId: trailer.doorId ? String(trailer.doorId).replace(/[^\w\-]/g, '').slice(0, 50) : null,
      createdAt: String(trailer.createdAt || new Date().toISOString()).slice(0, 50),
      dwellResets: Array.isArray(trailer.dwellResets) ? trailer.dwellResets.slice(0, 100) : []
    });
  }
  ...
  for (const slot of data.yardSlots) {
    sanitized.yardSlots.push({
      id: String(slot.id).replace(/[^\w\-]/g, '').slice(0, 50),
      number: Math.floor(Number(slot.number)) || 0,
      trailerId: slot.trailerId ? String(slot.trailerId).replace(/[^\w\-]/g, '').slice(0, 50) : null
    });
  }
...
app.post('/api/archives/restore', requireAuth, async (req, res) => {
  try {
    ...
    if (!validation.valid) {
      return res.status(400).json({ error: `Invalid archive: ${validation.error}` });
    }
    ...
    const backupPath = path.join(DATA_DIR, 'archives', `auto-backup-before-restore-${backupTimestamp}.json`);
